const text = [
  ...
  form.addEventListener("submit", function(e) {
    //prevent the default behaviour of form - sending the info to server and not console
    e.preventDefault();

   //parseInt return the value as a number (blue) instead of a string (black)
    const value = parseInt(amount.value);
    // console.log(typeof value)
   

    //empty value || negative value || > 9 value    

    if(isNaN(value) || value <= 0 || value > text.length) {

        const randomP = Math.floor(Math.random() * text.length); 
        result.innerHTML = `<p class="result">${text[randomP]}</p>`
    } 
    else {
        //create a temp array from the original array, starting from 0
